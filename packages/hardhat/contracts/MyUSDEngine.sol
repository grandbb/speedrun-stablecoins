...
contract MyUSDEngine is Ownable {
    ...
    function _getCurrentExchangeRate() internal view returns (uint256) {
        ...
        uint256 interest = (totalDebt * borrowRate * timeElapsed) / (SECONDS_PER_YEAR * 10000);
    ...
    function calculatePositionRatio(address user) public view returns (uint256) {
        ...
        return (calculateCollateralValue(user) * 100) / debt;
    }
    ...
    function mintMyUSD(uint256 mintAmount) public {
        ...
        _accrueInterest();
        uint256 shares = (mintAmount * PRECISION) / debtExchangeRate;
    ...
    function repayUpTo(uint256 amount) public {
        ...
        _accrueInterest();
        uint256 debt = getCurrentDebtValue(msg.sender);
        uint256 repayment = amount < debt ? amount : debt;
        if (repayment == 0) revert Engine__InvalidAmount();
        uint256 shares = repayment == debt ? s_userDebtShares[msg.sender] : (repayment * PRECISION) / debtExchangeRate;
        s_userDebtShares[msg.sender] -= shares;
        totalDebtShares -= shares;
        i_myUSD.burnFrom(msg.sender, repayment);
        emit DebtSharesBurned(msg.sender, repayment, shares);
    }
    ...
    function liquidate(address user) external {
        ...
        uint256 collateralForLiquidator = s_userCollateral[user];
        uint256 bonus = (collateralForLiquidator * LIQUIDATOR_REWARD) / 100;
        uint256 payout = collateralForLiquidator + bonus;
        // Cap the bonus at available collateral, leaving no stranded collateral in an insolvent position.
        if (payout > collateralForLiquidator) payout = collateralForLiquidator;
        totalDebtShares -= s_userDebtShares[user];
        s_userDebtShares[user] = 0;
        s_userCollateral[user] = 0;
        (bool success,) = payable(msg.sender).call{value: payout}("");
        if (!success) revert Engine__TransferFailed();
        emit Liquidation(user, msg.sender, payout, debt, i_oracle.getETHMyUSDPrice());
    }
}
